 ...
 function Connect4(p1, p2, gameId) 
 {
     ...
     {
         //JSON.stringify({numTurns: self.turn, player: p, turn: o.dataset})
         var httpRequest = new XMLHttpRequest();
         var location = '/send_data/' + this.game.gameId;
         httpRequest.open('POST', location, true);
         httpRequest.setRequestHeader('Content-Type', 'application/json');
         httpRequest.send(update);
     }

     function poller()
     {
         var httpRequest = new XMLHttpRequest();
         var location = '/poller/' + this.game.gameId + '/' + this.game.turn;
         httpRequest.onreadystatechange = function () 
         {
             if(httpRequest.readyState == 4 && httpRequest.status == 200)
             {
                 var jsonList = JSON.parse(httpRequest.responseText);
                 for (let i = 0; i < jsonList.length; i++)
                 {
                    var p = JSON.parse(jsonList[i])['player'];
                    var openCells = document.querySelectorAll('td.token-white.token[data-col="' + JSON.parse(jsonList[i])['turn']['col'] + '"]');
     ...
     var handleColumnClick = function(self) {
         return function(e) {
             ...
             var data;
             // place token
             var openCells = document.querySelectorAll('td.token-white.token[data-col="' + el.dataset.col + '"]'); //white cells.
             if (openCells.length > 0) {
                 var o = openCells[openCells.length - 1];
                 p.isRedToken ? o.classList.add('token-red') : o.classList.add('token-black'); //red token for the first.
                 p.isRedToken ? o.dataset.color = 'red' : o.dataset.color = 'black';
                 o.classList.remove('token-white');
                 ...
                 var data = JSON.stringify({numTurns: self.turn, player: p, turn: o.dataset})
     ...
     this.titleWin = function() {
         if (this.gameOver) {
             ...
             if (this.p1.winner) {
                 ...
                 localStorage.setItem((this.p1.id + '_' + this.gameId), (this.p1.turn));
             } 
             else 
             {
                 winText = winText + ": " + this.p2.name + " Wins!";
                 localStorage.setItem((this.p2.id + '_' + this.gameId), (this.p2.turn));
             }
     ...
     if (this.restoreGame()) {
         ...
         this.p1 = cached.p1;
         this.p2 = cached.p2;
         localStorage.clear(); //added here.
         
     }

     document.addEventListener('DOMContentLoaded', function(self) {
         return function(e) {
             //alert("first thing here.");
             //alerts are starting from this function so this is the beginning.
             self.makeBoard();
